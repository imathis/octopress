...
Octopress.twitter = function(options) {
  var opts = {
    count:        10,
    show_replies: false,
    target:       '#tweets'
  ...
  function render(tweets, user) {
    var content = '';
    for (var t in tweets) {
      content += '<li><p><a href="http://twitter.com/' + user + '/status/' + tweets[t].id_str + '">' +
                 prettyDate( tweets[t].created_at ) + '</a>' +
