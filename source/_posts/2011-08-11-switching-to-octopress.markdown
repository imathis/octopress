...
Octopress is a framework around [Jekyll](http://jekyllrb.com/ "Jekyll") and one of the features it adds is a [Rakefile](http://en.wikipedia.org/wiki/Rake_(software) "Rakefile") with helpful tasks predefined. The first rake task you'll make use of, {% codeblock %}rake install %} installs the default Octopress theme. There are also tasks to generate your site, deploy it (via rsync), and to create new pages or posts. 
